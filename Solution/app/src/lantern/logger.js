/**
 * Event logger — writes in FEG's own schema.
 *
 * Every column below is taken from top_casino_users_event_logs.csv, in the same
 * order, with the same event names and the same null convention. That is the
 * whole point: rows captured here can be concatenated onto the real export and
 * fed to the same training job without a mapping step.
 *
 *   node Solution/scripts/extract-data.mjs        # real logs  -> features
 *   (export from the Monitor tab)                 # this app   -> same shape
 *
 * Nothing leaves the device on its own. The CSV is produced only when someone
 * presses Export, which keeps the on-device promise in §4 of the architecture
 * intact even though the file is for training.
 */
...
/**
 * Turn one Lantern event into one FEG-schema row.
 * @param ev     the event as emitted onto the bus
 * @param ctx    { sessionId, playerId, route, riskState, archetype }
 */
export function toRow(ev, ctx) {
  ...
  // Lantern's own read of the moment — the label a model would train against.
